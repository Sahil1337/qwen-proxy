/**
 * Streaming. Thinking tokens arrive as `delta.reasoning_content`, answer
 * tokens as `delta.content`; the final chunk carries usage and meetiq.
 * With `tools` or `response_format` the proxy buffers and sends one chunk.
 *
 *   bun examples/05-streaming.ts
 */
import { MODEL, stream } from './lib.js';

let phase = '';
for await (const chunk of stream('/v1/chat/completions', {
  model: MODEL,
